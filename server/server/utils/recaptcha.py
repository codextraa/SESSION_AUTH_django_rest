...
def verify_recaptcha_token(
    ...
    if recaptcha_version == "v2":
        recaptcha_site_key = settings.RECAPTCHA_SITE_KEY_V2  #frontend uses this to generate the recaptcha token 
    else:
        recaptcha_site_key = settings.RECAPTCHA_SITE_KEY_V3

    client = recaptchaenterprise_v1.RecaptchaEnterpriseServiceClient() #recaptcha's backend client
    ...
    assessment = recaptchaenterprise_v1.Assessment()   #made an assessment object and added event to it, which will be sent to recaptcha's backend for verification
    ...
    response = client.create_assessment(request)  #response from recaptcha's backend after verification
